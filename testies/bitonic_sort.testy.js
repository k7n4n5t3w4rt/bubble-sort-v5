...
// ------------------------------------------------------------------
import { bitonicSortFactory } from "../js/three-sorting/actions/bitonicSort.js";
...
// Ensure Bitonic Sort sorts ascending in-place using adjacent swaps, with no animation delay.
// We inject a no-op scheduler to avoid timers during tests.

testPromise(
  "bitonicSort.js: bitonicSort(...) sorts ascending (in-place)",
  async () => {
    const cubesState = {
      active: true,
      moving: false,
    ...
    const bitonicSort = bitonicSortFactory(noopScheduleRepeat);
    const result = await bitonicSort(cubesState, 1, 1, fakeAnime);
