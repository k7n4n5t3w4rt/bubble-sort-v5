...
// ------------------------------------------------------------------
import { gnomeSortFactory } from "../js/three-sorting/actions/gnomeSort.js";
...
// Ensure gnomeSort sorts ascending in-place using adjacent swaps, with no animation delay.
// We inject a no-op scheduler to avoid timers during tests.

testPromise(
  "gnomeSort.js: gnomeSort(...) sorts ascending (in-place)",
  async () => {
    const cubesState = {
      active: true,
      moving: false,
    ...
    const gnomeSort = gnomeSortFactory(noopScheduleRepeat);
    const result = await gnomeSort(cubesState, 1, 1, fakeAnime);
